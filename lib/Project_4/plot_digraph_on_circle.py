...
def plot_edges(vertices, es):
    for edge in es:
        ...
        if not edge.is_mutual():
            ...
            plot_arr = [arrowhead_left, arrowhead_tip, arrowhead_right, arrowhead_tip, arrowhead_tip2, arrowhead_left2, arrowhead_tip2, arrowhead_right2]
        
        plt.plot([p[0] for p in plot_arr], [p[1] for p in plot_arr])


def plot_digraph_on_circle(g, colormap=None):
    fig, ax = plt.subplots()
